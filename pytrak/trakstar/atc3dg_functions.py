...
# Get the directory where the pytrak package is located
# This file is in pytrak/trakstar/, so we go up one level to find the DLL
...
if sys.platform.startswith("linux"):
    ...
    # Windows: Use absolute path to DLL in pytrak directory
    if ctypes.sizeof(ctypes.c_voidp) * 8 == 64:
        dll_name = str(_MODULE_DIR / "ATC3DG64.DLL")
    else:
        dll_name = str(_MODULE_DIR / "ATC3DG.DLL")

print("loading trakstar library {0}".format(dll_name))
try:
    _api = ctypes.CDLL(dll_name)
except OSError as e:
    print("ERROR: Can't find trakstar library: {0}".format(e))
    print("  Attempted path: {0}".format(dll_name))
    print("  Module directory: {0}".format(_MODULE_DIR))
    print("  DLL should be located at: {0}".format(_MODULE_DIR / "ATC3DG64.DLL"))
    _api = None
    # Don't raise here - allow module to load but functions won't be defined
    # This allows better error handling in the interface layer
except Exception as e:
    print("ERROR: Unexpected error loading trakstar library: {0}".format(e))
    print("  Attempted path: {0}".format(dll_name))
    _api = None
