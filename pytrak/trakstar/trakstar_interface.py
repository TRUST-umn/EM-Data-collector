...
class TrakSTARInterface(object):
    ...
    def initialize(self):
        ...
        # Check if API functions are available (DLL was loaded successfully)
        if not hasattr(api, 'InitializeBIRDSystem'):
            error_msg = (
                "TrakSTAR DLL not loaded. The ATC3DG64.DLL library could not be found or loaded.\n"
                "Please ensure:\n"
                "  1. ATC3DG64.DLL exists in the pytrak/ directory\n"
                "  2. The DLL is compatible with your system architecture (64-bit)\n"
                "  3. Any required dependencies for the DLL are installed\n"
                "  4. You have appropriate permissions to load the DLL"
            )
